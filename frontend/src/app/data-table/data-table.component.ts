...
export class DataTableComponent implements OnInit {
    ...
    getRows(): DataTableRowModel[] {
      const dataRows: DataTableRowModel[] = [];
      ...
      for (let i1 = 0; i1 < this.dataTable.dataRows.length; i1++) {
        const row = this.dataTable.dataRows[i1];
        let rowFound = false;

        for (let i2 = 0; i2 < row.cells.length; i2++) {
          const cell = row.cells[i2];

          for (let i3 = 0; i3 < cell.items.length; i3++) {
            const item = cell.items[i3];
    ...
    sortByCol(colId: string) {
      const currDir = this.dataTable.headerRow.sortDir;
      const currCol = this.dataTable.headerRow.sortCol;
      let newDir = 'asc';
    ...
    private sortRows(dataRows: DataTableRowModel[]): DataTableRowModel[] {
      const col = this.dataTable.headerRow.sortCol;
      const dir = this.dataTable.headerRow.sortDir;
      const headerCells = this.dataTable.headerRow.cells;
